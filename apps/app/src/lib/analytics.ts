...
async function safeTrack<T extends MetricData[] | PageViewData[]>(
  ...
  try {
    ...
    return (type === "page_view" ? [] as PageViewData[] : [] as MetricData[]) as T;
  }
}

export async function getAnalytics(): Promise<AnalyticsData> {
  const [pageViews, visitors, sessions] = await Promise.all([
    safeTrack<MetricData[]>("page_view", {
      interval: "1d",
      limit: 30,
    }, "metric"),
    safeTrack<MetricData[]>("visitor", {
      interval: "1d",
      limit: 30,
    }, "metric"),
    safeTrack<MetricData[]>("session", {
      interval: "1d",
      limit: 30,
    }, "metric"),
  ]);

  const topPages = await safeTrack<PageViewData[]>("page_view", {
    groupBy: "page",
    interval: "30d",
    limit: 5,
  }, "page_view");

  const engagement = await safeTrack<MetricData[]>("engagement", {
    interval: "30d",
  }, "metric");

  const bounceRate = await safeTrack<MetricData[]>("bounce", {
    interval: "30d",
  }, "metric");

  const returnUsers = await safeTrack<MetricData[]>("return_user", {
    interval: "30d",
  }, "metric");
